...
# This script generates FIGURE 14B of the DataPaper published in Earth Science System Data:
...
echo "35.7 36.9" |read VL1 VL2
echo "41.5 39.55" |read VL3 VL4
...
# 0       255/0/0       0.5      255/255/0
# 0.5     255/255/0     400     0/255/0
# EOF

#See map_time.cpt
# cat << EOF > time_palette.cpt
# -1500   25/175/255   0       205/255/162
# 0       205/255/162  0.5     255/189/87
# 0.5     255/189/87   400   255/124/124
# EOF
...
#
# # Aggiunta delle etichette A e A'
# echo "0 1 A" | gmt pstext -R0/$dist/-$vprof/0 -JX$vx/$valt -F+f50p,Helvetica-Bold,black+jLB -O -K >> $OUT
# echo "$dist 1 A'" | gmt pstext -R0/$dist/-$vprof/0 -JX$vx/$valt -F+f50p,Helvetica-Bold,black+jLB -O >> $OUT

# Plot degli eventi
awk '{print $1, $2, -$3, $5, $4*0.2}' $EQK | sort -k4,4n | gmt project -C$VL1/$VL2 -E$VL3/$VL4 -Fpz -W-$spes/$spes -V -Lw -Q | gmt psxy -R0/$dist/-$vprof/0 -JX$vx/$valt -Bxa40g20+l"Distance along section (km)" -Bya10g5+l"Depth (km)" -BWSne -W0.5p,black -Sc -Ctime_palette.cpt -X8 -Y15 -K > $OUT

# Aggiunta delle etichette A e A' con un piccolo spostamento
echo "0 -1 A" | gmt pstext -R0/$dist/-$vprof/5 -JX$vx/$valt -F+f50p,Helvetica-Bold,black+jLT -Dj-1c/-4c -O -K -N >> $OUT
echo "$dist -1 A'" | gmt pstext -R0/$dist/-$vprof/5 -JX$vx/$valt -F+f50p,Helvetica-Bold,black+jRT -Dj-1c/-4c -O -N >> $OUT
