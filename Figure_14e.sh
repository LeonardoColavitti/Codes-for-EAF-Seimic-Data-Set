...
#   - Automatically adds section labels D and D' at the ends
#
# Requirements :
...
#
# Output :
#   - PostScript file: section_DD-time.ps
#
# Usage :
#   $ ./section_DD-time.zsh
#
...
echo "39.1 40.15" |read VL1 VL2
echo "41.8 37.9" |read VL3 VL4
...
#awk '{print $1, $2, -$3, $5, $4*0.2}' $EQK | sort -k4,4n | gmt project -C$VL1/$VL2 -E$VL3/$VL4 -Fpz -W-$spes/$spes -V -Lw -Q | gmt psxy -R0/$dist/-$vprof/0 -JX$vx/$valt -Bxa40g20+l"Section length (km)" -Bya10g5+l"Depth (km)" -BWSne -W0.5p,black -Sc -Ctime_palette.cpt -X8 -Y15 -K > $OUT
...
echo "0 -1 D" | gmt pstext -R0/$dist/-$vprof/5 -JX$vx/$valt -F+f50p,Helvetica-Bold,black+jLT -Dj-1c/-4c -O -K -N >> $OUT
echo "$dist -1 D'" | gmt pstext -R0/$dist/-$vprof/5 -JX$vx/$valt -F+f50p,Helvetica-Bold,black+jRT -Dj-1c/-4c -O -N >> $OUT
